...
result (MartianChessGame tablero puntosDeimos puntosPhobos actPlayer)
   | isFinished tablero = if puntosDeimos > puntosPhobos then [ Winner DeimosPlayer,  Loser PhobosPlayer] else if puntosDeimos < puntosPhobos then  [ Winner PhobosPlayer,  Loser DeimosPlayer] else  [Draw]
   | otherwise = []
...
next game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer) DeimosPlayer action@(Mover (xO, yO) (xD, yD))  --- Cambio de Jugador activo y le sumo los puntos--- El tablero le dejo vacio la casilla de origen
   ...
   where puntos= valorfichas (getElem xD yD tablero) --- Obtengo los puntos Correspondiente a la ficha de destino.
...
remaining game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer)=
      ( aux (recorrerTableroPuntos2 sub1), aux (recorrerTableroPuntos2 sub2))
         where sub1= [(f, c, getElem f c tablero) | f <- [1..4], c <- [1..4]]
               sub2= [(f, c, getElem f c tablero) | f <- [1..4], c <- [5..8]]
...
recorrerTableroPuntos2 :: [(Int,Int,Casilla)]  -> [Int]
recorrerTableroPuntos2 t  =  [m | (f, c, x) <- t, x /= Vacio, m <- valorfichasContrl x]
