...
import Data.Char
import Data.Matrix(Matrix,getCol,getRow,toLists,fromLists)
...
actions juego
   ...
   where jugadorActual = activePlayer juego
         posicionesDisponibles = map ((+1).length.takeWhile (`elem` "XO")) (secciones juego)
...
next juego (jugador,(HijaraAction(seccion, posicion)))
   ...
   |accionPosibleSeccion seccionCambio movimiento = HijaraGame (fromLists (seccionesH tableroResultante))
...
readAction accion = HijaraAction(read seccion ::Int, read posicion::Int)
   where espacio = getIndex ' ' accion
...
cantidadDeFichas juego = length [casilla | casilla <- (valores juego), elem casilla "XO"] 
...
fichaJugador YellowPlayer = 'O'
fichaJugador BluePlayer = 'X'

valores :: HijaraGame -> String
valores tablero = concat (secciones tablero)

secciones :: HijaraGame -> [String]
secciones (HijaraGame juego) = concat (toLists juego)
...
accionPosibleSeccion :: [Char] -> HijaraAction -> Bool
accionPosibleSeccion unaSeccion (HijaraAction (seccion,posicion))
   ...
   |otherwise = True
   where movimiento = HijaraAction (seccion,posicion)
         posicionesLlenas = filter (`elem` "XO") unaSeccion
...
puntosMismaPosicion ficha juego = foldl1 (+) (map (puntuarFichaEnValores 10 ficha) lugatesIguales)
   where lugatesIguales = concat (map mismaPosicionEnSecciones (seccionesHVD (secciones juego)))
