USE findelmundoalfin;

CREATE TABLE prediccion_fin_mundo(
	ID_evento		INT,
	Nombre_Evento	VARCHAR(1000),
	Fecha_Evento	DATETIME,
	Descripcion_Evento	VARCHAR(1000),
	Dias_Faltantes		INT,
	Fuente_Prediccion	VARCHAR(1000)
)
...
SELECT	E.ID_Evento,
		...
		DATEDIFF(DAY, GETDATE(), E.Fecha_Evento),
		'Tabla Eventos Apocalípticos'
FROM eventos_apocalipticos E

SELECT AVG(P.Dias_Faltantes)
FROM prediccion_fin_mundo P
GROUP BY Fecha_Evento

SELECT COUNT(*) AS EventosConD
FROM prediccion_fin_mundo P
WHERE P.Nombre_Evento LIKE('D%')

SELECT COUNT(*) AS EventosConA
FROM prediccion_fin_mundo P
WHERE P.Nombre_Evento LIKE('A%')

select AVG(p.Dias_Faltantes), p.Decada AS Desde, p.Decada + 9 AS Hasta
from (select floor(YEAR(P.Fecha_Evento) / 10) * 10 as Decada, P.Dias_Faltantes
      from prediccion_fin_mundo P) p
group by Decada

select COUNT(p.Nombre_Evento) AS EventosConD, p.Decada AS Desde, p.Decada + 9 AS Hasta
from (SELECT floor(YEAR(P.Fecha_Evento) / 10) * 10 as Decada, P.Nombre_Evento
      FROM prediccion_fin_mundo P
	  WHERE Nombre_Evento LIKE('D%')) p
group by Decada

select COUNT(p.Nombre_Evento) AS EventosConA, p.Decada AS Desde, p.Decada + 9 AS Hasta
from (SELECT floor(YEAR(P.Fecha_Evento) / 10) * 10 as Decada, P.Nombre_Evento
      FROM prediccion_fin_mundo P
	  WHERE Nombre_Evento LIKE('A%')) p
group by Decada
