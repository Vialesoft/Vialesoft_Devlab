...
activePlayer tablero
   |fichas == 60 = Nothing --Juego terminado
...
readAction accion@[a,' ', b] = HijaraAction(read seccion ::Int, read posicion::Int)
   where espacio = fromJust (elemIndex ' ' accion)
...
puntosTotales jugador juego = puntosSeccion ficha juego + puntosMismaPosicion ficha juego + puntosEscalera ficha juego + puntosPorPosicion ficha juego
   where ficha = fichaJugador jugador

puntosPorPosicion :: Char -> HijaraGame -> Int
puntosPorPosicion ficha juego = foldl1 (+) (map (puntuarFichaPorPosicion ficha) (secciones juego))

puntuarFichaPorPosicion :: Char -> String -> Int
puntuarFichaPorPosicion ficha valores =
   if((length valores) == 4) then
      if(valores !! 3 == ficha) then
         4 + (puntuarFichaPorPosicion ficha (init valores))
      else
         0 + (puntuarFichaPorPosicion ficha (init valores))
   else if((length valores) == 3) then
      if(valores !! 2 == ficha) then
         3 + (puntuarFichaPorPosicion ficha (init valores))
      else
         0 + (puntuarFichaPorPosicion ficha (init valores))
   else if((length valores) == 2) then
      if(valores !! 1 == ficha) then
         2 + (puntuarFichaPorPosicion ficha (init valores))
      else
         0 + (puntuarFichaPorPosicion ficha (init valores))
   else if((length valores) == 1) then
      if(valores !! 0 == ficha) then
         1 + (puntuarFichaPorPosicion ficha (init valores))
      else
         0 + (puntuarFichaPorPosicion ficha (init valores))
   else 
      0
...
--Y así finaliza el conmovedor código de Samuel, un juego para disfrutar un sábado a la tarde, en compañía de un té con galletitas :)