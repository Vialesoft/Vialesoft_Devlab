...
actions juego 
   |(jugadorActual == Nothing) || (cantidadDeFichas juego == 60) = [(YellowPlayer,[]),(BluePlayer,[])]
...
isValidAction :: HijaraGame -> (HijaraPlayer, HijaraAction) -> Bool
isValidAction g (p,a) = head [elem a as | (p, as) <- actions g]
...
puntosPorPosicion ficha juego = foldl1 (+) ([if (seccion!!posicion) == ficha then posicion else 0 | seccion<-seccionesJuego, posicion<-[0..3]])
   where seccionesJuego = seccionesJuego
