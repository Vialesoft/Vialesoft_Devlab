...
import Data.Matrix -- Use command cabal install --lib matrix
import System.Random
import System.IO.Unsafe

beginning :: MartianChessGame
beginning = MartianChessGame (fromLists [[Queen, Queen, Drone, Empty, Empty, Empty, Empty, Empty], [Queen,Drone,Pawn_,Empty,Empty,Pawn_,Pawn_,Drone], [Drone,Pawn_,Pawn_,Empty,Empty,Pawn_,Drone,Queen], [Empty,Empty,Empty,Empty,Empty,Drone,Queen,Queen]]) (Just DeimosPlayer) (0,0) 250

data MartianChessPlayer = DeimosPlayer | PhobosPlayer deriving (Eq, Show, Enum, Bounded)
data MartianChessGame = MartianChessGame (Matrix MartianChessSquare) (Maybe MartianChessPlayer) (Int,Int) Int deriving (Eq, Show)
...
_actions (MartianChessGame board player _ _)
  | fromJust player == DeimosPlayer = concat [getActionsByCoordinate board DeimosPlayer (x,y) | x<-[1..4], y<-[1..4]]
  | fromJust player == PhobosPlayer = concat [getActionsByCoordinate board PhobosPlayer (x,y) | x<-[1..4], y<-[5..8]]

getActionsByCoordinate :: (Matrix MartianChessSquare) -> MartianChessPlayer -> (Int, Int) -> [MartianChessAction]
getActionsByCoordinate board player (x, y) 
  | board ! (x, y) == Empty = []
  | otherwise = map (\(cd, p) -> MartianChessAction (x, y) cd p) (concat (map (\((xf, yf), (dx, dy)) -> movePieceToDestination board player (x, y) (dx, dy) (xf, yf)) (getDestinationsByPiece (board ! (x, y)) (x,y))))
...
showMat (MartianChessGame board _ _ _) = putStrLn (prettyMatrix board)

actionToString :: MartianChessGame -> MartianChessAction -> String
actionToString (MartianChessGame board player (pd, pp) _) (MartianChessAction ci cf p) = "Move " ++ (show (board ! ci)) ++ " from " ++ (show ci) ++ " to " ++ (show cf) ++ " for " ++ (show p) ++ " points."
...
applyAction (MartianChessGame board player (pd, pp) n) (MartianChessAction ci cf p)
  | fromJust player == DeimosPlayer = (MartianChessGame (setElem Empty ci (setElem (board ! ci) cf board)) (Just PhobosPlayer) (pd + p, pp) (n-1))
  | fromJust player == PhobosPlayer = (MartianChessGame (setElem Empty ci (setElem (board ! ci) cf board)) (Just DeimosPlayer) (pd, pp + p) (n-1))
...
activePlayer (MartianChessGame board player p n)
  | n <= 0 = Nothing
  | hasFinished (MartianChessGame board player p n) = Nothing
  | otherwise = player

actions :: MartianChessGame -> [(MartianChessPlayer, [MartianChessAction])]
actions (MartianChessGame board player p n) = [(fromJust player, (_actions (MartianChessGame board player p n)))]
...
result (MartianChessGame board player (pd, pp) n) 
...
hasFinished (MartianChessGame board _ _ _)
...
showGame m@(MartianChessGame board player points moves) = (prettyMatrix (((fromLists [[X 1],[X 2],[X 3],[X 4]]) <|> board) <-> (fromLists [[Chess, Y 1, Y 2, Y 3, Y 4, Y 5, Y 6, Y 7, Y 8]]))) ++ "\r\nactive player -> " ++ (show player) ++ " points: " ++ (show points) ++ " moves left: " ++ (show moves) ++ (show (movesByScore m))

movesByScore :: MartianChessGame -> [(MartianChessAction, Int)]
movesByScore game = filter (\(_, p) -> p > 0) (map(\(MartianChessAction ci cf p) -> ((MartianChessAction ci cf p) , p)) (_actions game))
...
_runMatch rng ags@(ag1, ag2) g
  ...
  | otherwise = 
    ...
    in _runMatch (snd (System.Random.next rng)) ags (MartianChess.next g (fromJust(activePlayer g)) (ag rng g))
