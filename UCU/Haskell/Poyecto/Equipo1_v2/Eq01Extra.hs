...
beginning :: MartianChessConfig -> MartianChessGame
beginning (MartianChessConfig listaPuntos maxRondas) = MartianChessGame (fromLists lista) 0 0 DeimosPlayer listaPuntos 0 0 maxRondas
...
result (MartianChessGame tablero puntosDeimos puntosPhobos actPlayer puntos maxMovimientosDeimos maxMovimientosPhobos maxMovTotal )
   | (isFinished tablero) || (maxMovimientosDeimos>=maxMovTotal) || (maxMovimientosPhobos>=maxMovTotal) =
      if puntosDeimos > puntosPhobos then [ Winner DeimosPlayer,  Loser PhobosPlayer] else if puntosDeimos < puntosPhobos then  [ Winner PhobosPlayer,  Loser DeimosPlayer] else  [Draw]
   | otherwise = []
...
showGame (MartianChessGame tablero puntosDeimos puntosPhobos actPlayer puntos maxMovimientosDeimos maxMovimientosPhobos maxMovTotal)
   = "\n    1     2     3     4     5     6     7     8 \n"
...
actions game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer puntos maxMovimientosDeimos maxMovimientosPhobos maxMovTotal) 
...
recorrerTableroPlayer t game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer puntos maxMovimientosDeimos maxMovimientosPhobos maxMovTotal) = [m | (f, c, x) <- t, x /= Vacio, m <- _movimientos (f, c, x) game]
...
_PosibleMover game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer puntos maxMovimientosDeimos maxMovimientosPhobos maxMovTotal) posicionDestino@(x,y)
...
_PosibleMoverDiagonal game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer puntos maxMovimientosDeimos maxMovimientosPhobos maxMovTotal) posicionOrigen@(xo,yo) posicionDestino@(x,y)
...
_PosibleMoverHV game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer puntos maxMovimientosDeimos maxMovimientosPhobos maxMovTotal) posicionOrigen@(xo,yo) posicionDestino@(x,y)
...
_movimientos coordenada@(x,y,Peon) game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer puntos maxMovimientosDeimos maxMovimientosPhobos maxMovTotal) 
   =  [Mover (x, y) dest | dest <- [(x-1,y-1), (x-1,y+1),(x+1,y-1),(x+1,y+1)], _esPuntoValido dest && _PosibleMover game dest]

_movimientos coordenada@(x,y,Dron) game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer puntos maxMovimientosDeimos maxMovimientosPhobos maxMovTotal) 
...
_movimientos coordenada@(x,y,Reina) game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer puntos maxMovimientosDeimos maxMovimientosPhobos maxMovTotal) 
...
next game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer puntosFichas maxMovimientosDeimos maxMovimientosPhobos maxMovTotal) DeimosPlayer action@(Mover (xO, yO) (xD, yD))  --- Cambio de Jugador activo y le sumo los puntos--- El tablero le dejo vacio la casilla de origen
   =    MartianChessGame (setElem Vacio (xO, yO) tableroResultadoParcial) (puntosDeimos+puntos) puntosPhobos PhobosPlayer puntosFichas (maxMovimientosDeimos+1) maxMovimientosPhobos maxMovTotal  
...
next game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer puntosFichas maxMovimientosDeimos maxMovimientosPhobos maxMovTotal) PhobosPlayer action@(Mover (xO, yO) (xD, yD))  --- Cambio de Jugador activo y le sumo los puntos--- El tablero le dejo vacio la casilla de origen
   =    MartianChessGame (setElem Vacio (xO, yO) tableroResultadoParcial) puntosDeimos (puntosPhobos+puntos) DeimosPlayer puntosFichas maxMovimientosDeimos (maxMovimientosPhobos+1) maxMovTotal
   where puntos= valorfichas puntosFichas (getElem xD yD tablero) --- Obtengo los puntos Correspondiente a la ficha de destino.
         tableroResultadoParcial= setElem (getElem xO yO tablero) (xD, yD) tablero --- Primero el lugar de destino ya le seteo la nueva ficha
...
runGame ags = do
  runMatch ags (beginning m)
   where m= MartianChessConfig [2,4,5] 5
...
runsConsoleGame :: IO [GameResult MartianChessPlayer]
runsConsoleGame = do
...
movesByScore game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer) 
   | actPlayer==DeimosPlayer = recorrerTableroPuntos sub1 game
   | otherwise = recorrerTableroPuntos sub1 game
      where sub1= [(f, c, getElem f c tablero) | f <- [1..4], c <- [1..4]]
            sub2= [(f, c, getElem f c tablero) | f <- [1..4], c <- [5..8]]

recorrerTableroPuntos:: [(Int,Int,Casilla)]  -> MartianChessGame-> [(MartianChessAction, Int)]
recorrerTableroPuntos t game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer) = [ (m,puntoDestino m game) | (f, c, x) <- t, x /= Vacio, m <- _movimientos (f, c, x) game  ]

puntoDestino:: MartianChessAction -> MartianChessGame-> Int
puntoDestino action@(Mover (xO, yO) (xD, yD)) game@(MartianChessGame tablero puntosDeimos puntosPhobos actPlayer)
   | (getElem xD yD tablero)==Peon= 1
   | (getElem xD yD tablero)==Dron= 2
   | (getElem xD yD tablero)==Reina= 3
   | otherwise= 0